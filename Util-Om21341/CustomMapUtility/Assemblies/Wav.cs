    ...
    public readonly struct WAV
    {
            ...
            public RIFF(BinaryReader BinReader)
            {
                ...
                if (ChunkID != 1179011410u)
                {
                    ...
                    try
                    {
                        ...
                        if (BinReader.ReadUInt16() != 17990u) goto retry;
        ...
        public readonly struct Fmt
        {
            public Fmt(BinaryReader BinReader)
            {
                ...
                if (BinReader.ReadUInt16() != 8308) goto retry;
        ...
        public readonly struct Data
        {
            public Data(BinaryReader BinReader)
            {
                ...
                if (BinReader.ReadUInt16() != 24948u) goto retry;
                BinReader.BaseStream.Seek(-4, SeekOrigin.Current);
        ...
        public WAV(Stream wav)
        {
            ...
            var channel = 0;
            var iteration = new int[3];
            var total = dataBlock.Subchunk2Size;
            if (bytesPerSample != 2) throw new NotImplementedException("Wav reader currently only supports 16 bit PCM");
            for (var i = 0; i < total; i += (int)bytesPerSample)
            {
                var audioData = BytesToFloat(dataBlock.data[i], dataBlock.data[i + 1]);
                if (channel == 0)
                {
                ...
                iteration[2]++;
                channel++;
                if (channel >= fmtBlock.NumChannels) channel = 0;
            }
        ...
        public ushort NumChannels => fmtBlock.NumChannels;
        public uint FileSize => riffBlock.ChunkSize + 8u;
        public uint FileSize_Audio => dataBlock.Subchunk2Size;
        public uint SampleRate => fmtBlock.SampleRate;
        public uint BytesPerSample => fmtBlock.BitsPerSample / 8u;
        public uint SampleCount => FileSize_Audio / BytesPerSample / NumChannels;
        public Fmt.Tag AudioFormat => (Fmt.Tag)fmtBlock.AudioFormat;

        public override string ToString()
        {
#pragma warning disable MA0011
            return string.Format("[WAV: RIFFtag={4}, NumChannels={0}, FileSize={1}, SampleRate={2}, SampleCount={3}]",
                NumChannels, FileSize, SampleRate, SampleCount, AudioFormat);
